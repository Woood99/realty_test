...
export const usePurchaseList = (role_id, namePage) => {
   ...
   const handleSubmitFn = data => {
      setIsLoading(true);
      setFilterCount(getCountOfSelectedFilterObj(data, ['defaultCity', 'userIsSeller', 'activeType'], { city: data.defaultCity }));
      ...
      sendPostRequest(`/${data.userIsSeller ? 'seller-api' : 'api'}/purchase-orders`, params).then(res => {
         setIsLoading(false);
         setData(res.data);
      });
   };
   ...
   }, [JSON.stringify(watchedValues), currentPage, currentCity.id, userIsSeller, activeType, location.pathname]);
