...
const ApartmentSidebar = () => {
   ...
   const myDiscountPrice = myDiscount ? (myDiscount.type === 'price' ? myDiscount.discount : (priceOld / 100) * myDiscount.discount) : null;
   ...
   const total_amount =
      ...
      (+myDiscountPrice || 0) +
      (priceByDiscount ? priceOld - priceByDiscount : 0);
   ...
   return (
      <Sidebar>
         <div className="flex flex-col white-block-small !pt-2">
            ...
            {!Boolean(userRole === ROLE_SELLER.name && purchaseId) && myDiscount && !isEmptyArrObj(myDiscount) && (
               <div className="mb-4">
                  <CountdownTimer date={dayjs(dayjs.unix(myDiscount.valid_till)).format('YYYY-MM-DD')} />
               </div>
            )}
            ...
            {Boolean(!Boolean(userRole === ROLE_SELLER.name && purchaseId) && total_amount) && (
               <div className="bg-pageColor p-1 rounded-lg mt-4">
                  <div className="my-2 px-3 flex flex-col gap-2 w-full">
                     {Boolean(myDiscountPrice) && (
                        <Chars justifyBetween={true} color="blue">
                           <span className="!text-dark !font-medium">Персональная скидка</span>
                           <span className="!text-dark !font-medium">{numberReplace(myDiscountPrice)} ₽</span>
                        </Chars>
                     )}
                     {priceByDiscount && (
                        <Chars justifyBetween={true}>
                           ...
                           <span className="!text-[#b57600] !font-medium">
                              <AnimatedNumber to={priceOld - priceByDiscount} />
                              &nbsp;₽
                           </span>
                        </Chars>
                     )}
