...
const RoomInfoCard = memo(({ data, room, userRole }) => {
   ...
   const priceByDiscount = priceByDiscountApartment(data.buildingDiscount, data.price, data.area);
   const cashbackValue = (data.price / 100) * (((data.cashback / data.price) * 100 || 0) + (data.buildingCashback?.value || 0));
      ...
      <div className={styles.RoomInfoCardRoot}>
         ...
         <div className="flex flex-col gap-1 md3:gap-3">
            <div className="flex items-center gap-4 md3:flex-col md3:items-start md3:gap-1">
               {data.oldPrice !== data.price && <ElementOldPrice>{numberReplace(data.oldPrice)} ₽</ElementOldPrice>}
               <h3 className={`title-3 ${styles.RoomInfoCardTitle}`}>{numberReplace(priceByDiscount || data.price)} ₽</h3>
         ...
         <div className="flex gap-4 md1:gap-3 md1:items-start relative md1:col-span-2 md1:mt-4">
            <TagCashback cashback={cashbackValue} increased={Boolean(data.buildingCashback)} prefix="Кешбэк" />
            ...
            {Boolean(data.buildingDiscount) && (
               <TagDiscount
                  discount={data.buildingDiscount.value}
                  type_id={data.buildingDiscount.type}
                  unit={data.buildingDiscount.unit}
                  prefix="Скидка"
                  tooltip
                  start_date={dayjs(data.buildingDiscount.start_date).format('DD.MM.YYYY')}
                  end_date={dayjs(data.buildingDiscount.end_date).format('DD.MM.YYYY')}
               />
            )}
