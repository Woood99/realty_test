...
import PlayerCardsDynamic from './components/PlayerCardsDynamic';
...
export const Shorts = ({ data, condition = true, startIndex, startData, single = false }) => {
   ...
   return (
      <div ref={modalContainerRef}>
         <div className="video-player shorts-player shorts-player-controls mt-8">
            <Swiper
               modules={[Navigation]}
               ref={swiperRef}
            ...
            <div className="self-start overflow-hidden rounded-[20px]">
               <div className="bg-white rounded-[20px] overflow-hidden">
                  ...
                  <div className="mt-4 order-1 md1:mt-[calc(56.25%+0px)] ">
                     <div className="flex items-start gap-4 justify-between px-4 pb-4 shadow-primary">
                        <div className="flex-grow">
                           ...
                           <PlayerTitle title={data.name} className="relative" classNameTitle="title-3" />
                           <PlayerAuthor
                              data={data}
                        ...
                        <Button
                           variant="secondary"
                           size={isDesktop ? 'Small' : '34'}
                           className="flex gap-2 items-center"
                           onClick={() => setIsShareModal(true)}>
                           <IconShareArrow className="fill-lightGray" width={isDesktop ? 14 : 12} height={isDesktop ? 14 : 12} />
...
export const ShortPlayer = ({ data, currentId, classNamePlayer = '' }) => {
   ...
   return (
      <div className="h-full w-full cursor-pointer">
         {isActiveSidebar && isDesktop && <PlayerCardsDynamic data={data} player={element} />}
         <div data-vjs-player>
         ...
         <PlayerTitle
            title={data.name}
            className={`${data.cards.length ? 'bottom-[106px]' : 'bottom-[20px]'} left-4`}
            building_id={data.building_id}
            building_name={data.building_name}
            type="short"
         />
         <PlayerCards data={data} onClick={() => setIsActiveSidebar(prev => !prev)} />
         {userInfo?.role?.id === ROLE_BUYER.id &&
            element &&
            !isEmptyArrObj(data.interactiveEl) &&
         ...
         <ModalWrapper condition={isActiveSidebar && !isDesktop}>
            <Modal
               ...
               <PlayerCardsDynamic data={data} player={element} />
            </Modal>
         </ModalWrapper>
      </div>
   );
