...
export const useChatMessagesPagination = options => {
   ...
   const getDialogAction = async (id, page) => {
      const { data } = await sendPostRequest(`/api/dialogs/${id}`, { limit: 100, page });
   ...
   useEffect(() => {
      ...
      const handleScrollUp = throttle(async () => {
         try {
            if (!(scrollableElement.scrollTop <= 150)) return;
            ...
            const data = await getDialogAction(current.id, current.current_page - 1);
