...
import { ChatMainTextarea, ChatMenu, ChatPresent, ChatSmile, ChatVoiceRecorder } from '.';

const ChatBottom = () => {
   const { chatBottomRef, isEdit, setIsEdit, messageText, sendMessage, isVoiceRecording, setMessageText, variantChat, filesUpload, setFilesUpload } =
   ...
   return (
      <div className="flex items-center gap-2 mmd1:max-w-[920px] mmd1:mx-auto" ref={chatBottomRef}>
         {!isVoiceRecording && (
            <>
               ...
               {checkForSend && (
                  <button className={styles.ChatBtnBlue} onClick={() => sendMessage()}>
                     {isEdit ? <IconCheckedSecond className="fill-[#fff]" /> : <IconSend width={24} height={24} className="fill-[#fff]" />}
                  </button>
               )}
            </>
         )}
