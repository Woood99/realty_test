...
import { IconEllipsis, IconMegaphone, IconUsers, IconUsers2 } from '../../../../ui/Icons';

const ChatDialogButton = memo(({ data, options }) => {
   const { sidebarMini, showPopper, setShowPopper } = options;

   const { currentDialog, setCurrentDialog, connectToChat, getDialog, setIsLoadingDialog, setIsVisibleBtnArrow, chatSearchDialog, setCachedDialog } =
   ...
   const handleButtonClick = () => {
      if (currentDialog.id === data.id) return;
               ...
               <div className="flex items-center gap-1 relative">
                  ...
                  <ChatTooltipDialog
                     options={{
                        ...
                        ElementTargetLayout: <IconEllipsis className={cn('pointer-events-none fill-[#fff]')} width={16} height={16} />,
                     }}
                  />
               </div>
            </div>
