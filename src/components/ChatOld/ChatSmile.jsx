...
export const SmileItem = ({ item, onClick }) => {
   return (
      <button type="button" className="hover:bg-primary700 p-2.5 rounded-lg" onClick={onClick}>
         {item}
      </button>
   );
};
...
const ChatSmile = ({ setMessageText }) => {
   const { chatRootRef, isOpenSmileMenu, setIsOpenSmileMenu, setIsOpenMenu } = useContext(ChatContext);

   const isDesktop = useSelector(getIsDesktop);

   useEffect(() => {
      const el = document.getElementById('chat-mobile-smile');
      if (el && chatRootRef.current) {
         chatRootRef.current.style.setProperty('--chat-smile-height', `${el.clientHeight}px`);
      }
   }, [isOpenSmileMenu]);
